...
static inline void decode_check_jalr_callret(Decode *s, int rd){
  uint32_t i = s->isa.inst.val;
  int rs1 = BITS(i, 19, 15);
  if(rd == 0){
    if(rs1 == 1){
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & ~((word_t) 1); decode_check_jalr_callret(s, rd));
